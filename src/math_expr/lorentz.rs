...
use super::{IndexRange, SummationIndex, MathExpr, BinaryOperator};
use permutohedron;
...
pub enum LorentzTensor {
    #[serde(rename = "C")]
    ChargeConjugation { i1: SpinorIndex, i2: SpinorIndex },
    Epsilon {mu1: LorentzIndex, mu2: LorentzIndex, mu3: LorentzIndex, mu4: LorentzIndex},
    Gamma {mu1: LorentzIndex, i2: SpinorIndex, i3: SpinorIndex},
    Gamma5 { i1: SpinorIndex, i2: SpinorIndex },
    #[serde(rename = "Identity")]
    KroneckerDelta { i1: SpinorIndex, i2: SpinorIndex },
    Metric {mu1: LorentzIndex, mu2: LorentzIndex},
    #[serde(rename = "P")]
    Momentum { mu1: LorentzIndex, particle: i64},
    ProjM { i1: SpinorIndex, i2: SpinorIndex },
    ProjP { i1: SpinorIndex, i2: SpinorIndex },
    Sigma {mu1: LorentzIndex, mu2: LorentzIndex, i3: SpinorIndex, i4: SpinorIndex},
}
impl LorentzTensor {
    fn get_component(&self, components: &SpinTensorComponents, indices: &Indices) -> Complex64 {
        match self {
            &LorentzTensor::ChargeConjugation {i1, i2} => components.charge_conjugation.get(indices[i1], indices[i2]),
            &LorentzTensor::Epsilon {mu1, mu2, mu3, mu4} => components.epsilon.get(indices[mu1], indices[mu2], indices[mu3], indices[mu4]),
            &LorentzTensor::Gamma {mu1, i2, i3} => components.gamma.get(indices[mu1], indices[i2], indices[i3]),
            &LorentzTensor::Gamma5 {i1, i2} => components.gamma5.get(indices[i1], indices[i2]),
            &LorentzTensor::KroneckerDelta {i1, i2} => components.identity.get(indices[i1], indices[i2]),
            ...
            &LorentzTensor::Sigma {mu1, mu2, i3, i4} => components.epsilon.get(indices[mu1], indices[mu2], indices[i3], indices[i4]),
...
impl SpinTensorComponents {
    pub fn load<P: AsRef<path::Path>>(path: P) -> Result<SpinTensorComponents, SpinComponentsError> {
        ...
        let mut components = SpinTensorComponents {
            ...
            epsilon,
            charge_conjugation: SpinTensorComponents::compute_charge_conjugation(&internal.charge_conjugation),
    ...
    fn compute_gamma5(relation: &SpinTensorRelation, components: &mut SpinTensorComponents) {
        ...
        for i in SpinorIndex::range() {
            indices.set_index(SummationIndex::Spinor { index: relation.spinor[0] }, i);
            for j in SpinorIndex::range() {
                indices.set_index(SummationIndex::Spinor {index: relation.spinor[1] }, j);
                let expr = expand_sums(&relation.expr, components, &mut indices).constant_propagation();
                match expr.get_value() {
                    None => panic!("BUG: Failed to compute ({},{}) component of gamma5: {:?}", i, j, expr),
                    Some(e) => {
                        if !e.is_zero() {
                            gamma5.insert(i, j, e);
                        }
    ...
    fn compute_projm(relation: &SpinTensorRelation, components: &mut SpinTensorComponents) {
        ...
        for i in SpinorIndex::range() {
            ...
            for j in SpinorIndex::range() {
                ...
                let expr = expand_sums(&relation.expr, components, &mut indices).constant_propagation();
                match expr.get_value() {
                    None => panic!("BUG: Failed to compute ({},{}) component of ProjM: {:?}", i, j, expr),
                    Some(e) => {
                        if !e.is_zero() {
                            projm.insert(i, j, e);
                        }
    ...
    fn compute_projp(relation: &SpinTensorRelation, components: &mut SpinTensorComponents) {
        ...
        for i in SpinorIndex::range() {
            ...
            for j in SpinorIndex::range() {
                ...
                let expr = expand_sums(&relation.expr, components, &mut indices).constant_propagation();
                match expr.get_value() {
                    None => panic!("BUG: Failed to compute ({},{}) component of ProjP: {:?}", i, j, expr),
                    Some(e) => {
                        if !e.is_zero() {
                            projp.insert(i, j, e);
                        }
    ...
    fn compute_sigma(relation: &SpinTensorRelation, components: &mut SpinTensorComponents) {
        ...
        let mut indices = Indices::new();
        for mu in LorentzIndex::range() {
            indices.set_index(SummationIndex::Lorentz { index: relation.lorentz[0] }, mu);
            for nu in LorentzIndex::range() {
                indices.set_index(SummationIndex::Lorentz { index: relation.lorentz[1] }, nu);
                for i in SpinorIndex::range() {
                    indices.set_index(SummationIndex::Spinor { index: relation.spinor[0] }, i);
                    for j in SpinorIndex::range() {
                        indices.set_index(SummationIndex::Spinor {index: relation.spinor[1] }, j);
                        let expr = expand_sums(&relation.expr, components, &mut indices).constant_propagation();
                        match expr.get_value() {
                            None => panic!("BUG: Failed to compute ({},{},{},{}) component of sigma: {:?}", mu, nu, i, j, expr),
                            Some(e) => {
                                if !e.is_zero() {
    ...
    fn compute_epsilon(sign: f64) -> TensorComponents4 {
        ...
        permutohedron::heap_recursive(&mut start, |permutation| {
            ...
            components.insert(permutation[0], permutation[1], permutation[2], permutation[3], value);
        });
        components
    }
}
...
fn expand_sums(expr: &MathExpr, components: &SpinTensorComponents, indices: &mut Indices) -> MathExpr {
    match expr {
        MathExpr::Sum { ref expr, index } => {
            ...
            let mut val = terms.next().expect("BUG: There has to be at least one value for an index!");
            for t in terms {
        ...
        MathExpr::BinaryOp { operator, left, right } => {
            MathExpr::BinaryOp {
                operator: *operator,
        ...
        MathExpr::Conditional { condition, if_true, if_false } => {
        ...
        MathExpr::Call { function, args } => {
            MathExpr::Call {
                ...
                args: args.iter().map(|a| expand_sums(a, components, indices)).collect(),
            }
        }
        _ => expr.clone(),
    }
...
impl TensorComponents2 {
    ...
    pub fn get(&self, i1: u8, i2: u8) -> Complex64 {
        *self.components.get(&(i1,i2)).unwrap_or(&Complex64::new(0f64,0f64))
    }
...
impl TensorComponents3 {
    ...
    pub fn get(&self, i1: u8, i2: u8, i3: u8) -> Complex64 {
        *self.components.get(&(i1,i2,i3)).unwrap_or(&Complex64::new(0f64,0f64))
    }
...
impl TensorComponents4 {
    ...
    pub fn get(&self, i1: u8, i2: u8, i3: u8, i4: u8) -> Complex64 {
        *self.components.get(&(i1,i2,i3,i4)).unwrap_or(&Complex64::new(0f64,0f64))
    }
...
impl fmt::Display for SpinComponentsError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            SpinComponentsError::IoError(ref err) => write!(f, "Failed to read the spin structures file: {}", err),
            SpinComponentsError::TomlConversionError(ref err) => write!(f, "Failed to parse the spin structures file: {}", err),
            SpinComponentsError::JsonConversionError(ref err) => write!(f, "Failed to parse the spin relations file: {}", err),
        }
    }
}
...
mod test {
    ...
    #[test]
    fn gamma5() {
        let components = super::SpinTensorComponents::load("models/common/spin_structures.toml").unwrap();
